...
#include "vendor/mesh_provision/user/debug.h"
#ifdef APP_DBG_EN
...
#define DBG_APP_SEND_HEX_ONE_BYTE(x)  Dbg_sendHexOneByte(x)
#else
#define DBG_APP_SEND_STR(x)
#define DBG_APP_SEND_INT(x)
#define DBG_APP_SEND_HEX(x)
#define DBG_APP_SEND_HEX_ONE_BYTE(x)
#endif
...
#else
#define BLT_RX_FIFO_SIZE        (MESH_DLE_MODE ? DLE_RX_FIFO_SIZE : 100)
...
int gateway_common_cmd_rsp(u8 code,u8 *p_par,u16 len )
{
	...
	*/
	...
	if(code == HCI_GATEWAY_CMD_SEND_CPS_INFO){
		...
		while(len){
			if(len > fifo_size){//continues packet
				head[2] = SAR_CONTINUE;
				my_fifo_push_hci_tx_fifo(p_buf,fifo_size, head, 3);
				len -= fifo_size;
				p_buf += fifo_size;
...
static void gateway_response_gw_appkey_cmd(u8 cmd, config_appkey_st_t* p_cfg_ak_st){

	gateway_common_cmd_rsp(cmd, (u8 *)p_cfg_ak_st, sizeof(config_appkey_st_t));
}
static u8 gateway_handle_gw_appkey_cmd(u16 code, cmd_appkey_t* p_cmd_ak)
{
	u8 st = ST_UNSPEC_ERR;

	if((code == APPKEY_DEL)||(code == APPKEY_UPDATE)||(code==APPKEY_ADD)){
		update_key_by_bridge_flag = TRUE;
		st = mesh_app_key_set(code, (u8*)p_cmd_ak->ak, p_cmd_ak->ak_idx, p_cmd_ak->nk_idx, TRUE);
		update_key_by_bridge_flag = FALSE;
	}else{
		st = ST_UNSPEC_ERR;
		return st;
	}
	config_appkey_st_t* p_cfg_ak_st = (config_appkey_st_t*)p_cmd_ak;
	p_cfg_ak_st->st = st;
	u8 rsp_cmd = ((code == APPKEY_DEL)?(HCI_GATEWAY_CMD_DELETE_APPKEY_STATUS):
			((code == APPKEY_UPDATE)?(HCI_GATEWAY_CMD_UPDATE_APPKEY_STATUS):(HCI_GATEWAY_CMD_ADD_APPKEY_STATUS)));
	gateway_response_gw_appkey_cmd(rsp_cmd, p_cfg_ak_st);
	return st;
}
...
void gateway_send_vc_node_infor(u16 ele_adr)
{
	VC_node_info_t * vc_node_info = get_VC_node_info(ele_adr, 1);
	if(vc_node_info != 0){
		gateway_common_cmd_rsp(HCI_CMD_GATEWAY_VC_NODE_INFOR_RSP, (u8 *)vc_node_info, sizeof(VC_node_info_t));
	}else{
		gateway_common_cmd_rsp(HCI_CMD_GATEWAY_VC_NODE_INFOR_RSP, (u8 *)&ele_adr, 2);
	}
}
...
extedned_get_t extended_get = {.cnt = 0, .get_last_t_ms = 0 };
...
int gateway_cmd_from_host_ctl(u8 *p, u16 len )
{
	...
	if(op_code == HCI_GATEWAY_CMD_START){
		...
		gateway_send_vc_node_infor(p_info->node_adr);
	}else if(op_code == HCI_GATE_WAY_CMD_GET_VC_NODE_INFOR){
		mesh_gw_get_node_infor_t *p_info = (mesh_gw_get_node_infor_t*)(p+1);
		gateway_send_vc_node_infor(p_info->node_adr);
	}
	else if(op_code == HCI_GATEWAY_CMD_GET_APPKEY){
		cmd_appkey_t* p_cmd_ak = (cmd_appkey_t*)(p + 1);
		mesh_app_key_t* p_app_key;
		p_app_key = mesh_app_key_search_by_index(p_cmd_ak->nk_idx, p_cmd_ak->ak_idx);
		gateway_common_cmd_rsp(HCI_GATEWAY_CMD_SEND_APPKEY, (u8 *)p_app_key, sizeof(mesh_app_key_t));
	}
	else if(op_code == HCI_GATEWAY_CMD_ADD_APPKEY){
		cmd_appkey_t* p_cmd_ak = (cmd_appkey_t*)(p + 1);
		gateway_handle_gw_appkey_cmd(APPKEY_ADD, p_cmd_ak);
	}
	else if(op_code == HCI_GATEWAY_CMD_UPDATE_APPKEY){
		cmd_appkey_t* p_cmd_ak = (cmd_appkey_t*)(p + 1);
		gateway_handle_gw_appkey_cmd(APPKEY_UPDATE, p_cmd_ak);
	}
	else if(op_code == HCI_GATEWAY_CMD_DELETE_APPKEY){
		cmd_appkey_t* p_cmd_ak = (cmd_appkey_t*)(p + 1);
		gateway_handle_gw_appkey_cmd(APPKEY_DEL, p_cmd_ak);
	}
	else if(op_code == HCI_GATEWAY_CMD_SET_LAST_UNICAST_ADR){
		int err;
		rsp_set_unicast_adr_last_t rsp_set_unicast_adr_last;
		rsp_set_unicast_adr_last.last_adr[0] = p[1];
		rsp_set_unicast_adr_last.last_adr[1] = p[2];
		err = VC_cmd_set_unicast_adr_last(p[1]|(p[2]<<8));
		if(err == 0){
			rsp_set_unicast_adr_last.stt = ST_SUCCESS;
		}else{
			rsp_set_unicast_adr_last.stt = ST_INVALID_ADR;
		}
		gateway_common_cmd_rsp(
				HCI_CMD_GATEWAY_SET_LAST_UNICAST_RSP,
				(u8 *)&rsp_set_unicast_adr_last,
				sizeof(rsp_set_unicast_adr_last_t)
				);
	}
	else if(op_code == HCI_GATEWAY_CMD_REBOOT){
		start_reboot();
	}
	else if(op_code == HCI_GATEWAY_CMD_PERIPHERAL) {
		// Periph_handleSerialIncommingMessage((u8*)&p[1], len - 1);
	}
	return 1;
}
