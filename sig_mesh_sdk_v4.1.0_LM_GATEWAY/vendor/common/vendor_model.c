...
#else
//
#define cb_vd_dev_opt_set           (0)
#define cb_vd_dev_opt_get           (0)
//
#define cb_vd_auto_trans_set        (0)
#define cb_vd_auto_trans_get        (0)
//
#define cb_vd_custom_rgb_effect_get (0)
#define cb_vd_custom_rgb_effect_set (0)
//
#define cb_vd_light_state_get       (0)
//
#define cb_vd_config_model_sub_set  (0)
//
#define cb_vd_scene_request         (0)
//
#define cb_vd_scene_request         (0)
//
#define cb_vd_scene_request_noack   (0)
//
#define cb_vd_execution_scene_set   (0)
//
#define cb_vd_test_led_set          (0)
//
#define cb_vd_light_music_scene_set (0)
//
#define cb_vd_config_node_set       (0)
//
#define cb_vd_light_speaker_set     (0)
#define cb_vd_light_speaker_get     (0)
...
int cb_vd_dev_opt_status(u8 *par, int par_len, mesh_cb_fun_par_t *cb_par){
	int err = 0;
	if(cb_par->model){  // model may be Null for status message
	    //model_client_common_t *p_model = (model_client_common_t *)(cb_par->model);
	}
...
int cb_vd_auto_trans_status(u8 *par, int par_len, mesh_cb_fun_par_t *cb_par){
	int err = 0;
	if(cb_par->model){  // model may be Null for status message
	    //model_client_common_t *p_model = (model_client_common_t *)(cb_par->model);
	}
...
int cb_vd_custom_rgb_effect_status(u8 *par, int par_len, mesh_cb_fun_par_t *cb_par){
	int err = 0;
	if(cb_par->model){  // model may be Null for status message
	    //model_client_common_t *p_model = (model_client_common_t *)(cb_par->model);
	}
...
int cb_vd_light_state_status(u8 *par, int par_len, mesh_cb_fun_par_t *cb_par){
	int err = 0;
	if(cb_par->model){  // model may be Null for status message
	    //model_client_common_t *p_model = (model_client_common_t *)(cb_par->model);
	}
...
int cb_vd_power_status(u8 *par, int par_len, mesh_cb_fun_par_t *cb_par){
	int err = 0;
	if(cb_par->model){  // model may be Null for status message
		//model_client_common_t *p_model = (model_client_common_t *)(cb_par->model);
	}
...
int cb_vd_config_model_sub_status(u8 *par, int par_len, mesh_cb_fun_par_t *cb_par){
	int err = 0;
	if(cb_par->model){  // model may be Null for status message
		//model_client_common_t *p_model = (model_client_common_t *)(cb_par->model);
	}
...
int cb_vd_scene_response(u8 *par, int par_len, mesh_cb_fun_par_t *cb_par){
	int err = 0;
	if(cb_par->model){  // model may be Null for status message
		//model_client_common_t *p_model = (model_client_common_t *)(cb_par->model);
	}
...
int cb_vd_execution_scene_response(u8 *par, int par_len, mesh_cb_fun_par_t *cb_par){
	int err = 0;
	if(cb_par->model){  // model may be Null for status message
		//model_client_common_t *p_model = (model_client_common_t *)(cb_par->model);
	}
...
int cb_vd_config_node_status(u8 *par, int par_len, mesh_cb_fun_par_t *cb_par){
	int err = 0;
	if(cb_par->model){  // model may be Null for status message
		//model_client_common_t *p_model = (model_client_common_t *)(cb_par->model);
	}
...
int cb_vd_light_speaker_status(u8 *par, int par_len, mesh_cb_fun_par_t *cb_par){
	int err = 0;
	if(cb_par->model){  // model may be Null for status message
		//model_client_common_t *p_model = (model_client_common_t *)(cb_par->model);
	}
...
mesh_cmd_sig_func_t mesh_cmd_vd_func[] = {
	...
	CMD_NO_STR(VD_RC_KEY_REPORT, 1, VENDOR_MD_LIGHT_C, VENDOR_MD_LIGHT_S, cb_vd_key_report, STATUS_NONE),
    #endif
        
    #if (FAST_PROVISION_ENABLE)
	CMD_NO_STR(VD_MESH_RESET_NETWORK, 0, VENDOR_MD_LIGHT_C, VENDOR_MD_LIGHT_S, cb_vd_mesh_reset_network, STATUS_NONE),
	CMD_NO_STR(VD_MESH_ADDR_GET, 0, VENDOR_MD_LIGHT_C, VENDOR_MD_LIGHT_S, cb_vd_mesh_get_addr, STATUS_NONE),
    ...
    // Luci power report
	{VD_NEMA_ENERGY_REPORT, 1, VENDOR_MD_LIGHT_S, VENDOR_MD_LIGHT_C, cb_vd_power_status, STATUS_NONE},
	...
	{VD_AUTO_TRANS_SET, 0, VENDOR_MD_LIGHT_C, VENDOR_MD_LIGHT_S, cb_vd_auto_trans_set, VD_AUTO_TRANS_STATUS},
	{VD_AUTO_TRANS_GET, 0, VENDOR_MD_LIGHT_C, VENDOR_MD_LIGHT_S, cb_vd_auto_trans_get, VD_AUTO_TRANS_STATUS},
	{VD_AUTO_TRANS_SET_NOACK, 0, VENDOR_MD_LIGHT_C, VENDOR_MD_LIGHT_S, cb_vd_auto_trans_set, STATUS_NONE},
	{VD_AUTO_TRANS_STATUS, 1, VENDOR_MD_LIGHT_S, VENDOR_MD_LIGHT_C, cb_vd_auto_trans_status, STATUS_NONE},

	// Custom RGB
	{VD_CUSTOM_RGB_EFFECT_GET, 0, VENDOR_MD_LIGHT_C, VENDOR_MD_LIGHT_S, cb_vd_custom_rgb_effect_get, VD_CUSTOM_RGB_EFFECT_STATUS},
	{VD_CUSTOM_RGB_EFFECT_SET, 0, VENDOR_MD_LIGHT_C, VENDOR_MD_LIGHT_S, cb_vd_custom_rgb_effect_set, VD_CUSTOM_RGB_EFFECT_STATUS},
	{VD_CUSTOM_RGB_EFFECT_SET_NOACK, 0, VENDOR_MD_LIGHT_C, VENDOR_MD_LIGHT_S, cb_vd_custom_rgb_effect_set, STATUS_NONE},
	{VD_CUSTOM_RGB_EFFECT_STATUS, 1, VENDOR_MD_LIGHT_S, VENDOR_MD_LIGHT_C, cb_vd_custom_rgb_effect_status, STATUS_NONE},
	#endif
	...
	// Add Group
	CMD_NO_STR(VD_CONFIG_MODEL_SUB_SET, 0, VENDOR_MD_LIGHT_C, VENDOR_MD_LIGHT_S, cb_vd_config_model_sub_set, VD_CONFIG_MODEL_SUB_STATUS),
	CMD_NO_STR(VD_CONFIG_MODEL_SUB_STATUS, 1, VENDOR_MD_LIGHT_S, VENDOR_MD_LIGHT_C, cb_vd_config_model_sub_status, STATUS_NONE),
	...
	// Vendor Execution Scene
	{VD_EXECUTION_SCENE_SET, 0, VENDOR_MD_LIGHT_C, VENDOR_MD_LIGHT_S, cb_vd_execution_scene_set, VD_EXECUTION_SCENE_RESPONSE},
	{VD_EXECUTION_SCENE_RESPONSE, 1, VENDOR_MD_LIGHT_S, VENDOR_MD_LIGHT_C, cb_vd_execution_scene_response, STATUS_NONE},

	// Vendor Configuration Node With ACK
	{VD_CONFIG_NODE_SET, 0, VENDOR_MD_LIGHT_C, VENDOR_MD_LIGHT_S, cb_vd_config_node_set, VD_CONFIG_NODE_STATUS},
	...
	{VD_LIGHT_SPEAKER_GET, 0, VENDOR_MD_LIGHT_C, VENDOR_MD_LIGHT_S, cb_vd_light_speaker_get, VD_LIGHT_SPEAKER_STATUS},
	...
	// Test Led
	{VD_LIGHT_AUTO_TEST_SET_NOACK, 0, VENDOR_MD_LIGHT_C, VENDOR_MD_LIGHT_S, cb_vd_test_led_set, STATUS_NONE},

	//
	{VD_CTL_MUSIC_SCENE_SET, 0, VENDOR_MD_LIGHT_C, VENDOR_MD_LIGHT_S, cb_vd_light_music_scene_set, STATUS_NONE},
	{VD_HSL_MUSIC_SCENE_SET, 0, VENDOR_MD_LIGHT_C, VENDOR_MD_LIGHT_S, cb_vd_light_music_scene_set, STATUS_NONE},

	// Reserved
	{VD_RESERVED_11, 0, VENDOR_MD_LIGHT_C, VENDOR_MD_LIGHT_S, 0, STATUS_NONE},
