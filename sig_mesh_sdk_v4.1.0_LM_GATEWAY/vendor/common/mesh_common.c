...
#include "../mesh_provision/user/debug.h"
...
#define DBG_MESH_COMMON_SEND_HEX_ONE_BYTE(x)  Dbg_sendHexOneByte(x)
#else
#define DBG_MESH_COMMON_SEND_STR(x)
#define DBG_MESH_COMMON_SEND_INT(x)
#define DBG_MESH_COMMON_SEND_HEX(x)
#define DBG_MESH_COMMON_SEND_HEX_ONE_BYTE(x)
#endif
...
MYFIFO_INIT(hci_rx_fifo, HCI_RX_FIFO_SIZE, 32);
#endif
#endif
...
int app_hci_cmd_from_usb_handle (u8 *buff, int n) // for both usb and uart
{
	...
	else if (HCI_CMD_USER_DEFINE == type)	//user defined command
	{
		...
		#else
		#if GATEWAY_ENABLE
       // Modify vendor command
		mesh_bulk_vd_cmd_par_t * p_bulk_cmd = (mesh_bulk_vd_cmd_par_t*)hci_data;
		u8 op_type = GET_OP_TYPE(p_bulk_cmd->op);
		if(OP_TYPE_VENDOR == op_type){
		// Old bridge version using default op_rsp
			if(p_bulk_cmd->op_rsp == VD_GROUP_G_STATUS) {
				p_bulk_cmd->op_rsp = OP_RESPONSE_UNKNOWN;
				p_bulk_cmd->tid_pos = VENDOR_NO_TID;
			}
		 }
		// if the command is the node reset cmd ,need to proc the vc node info part
		ret = mesh_bulk_cmd((mesh_bulk_cmd_par_t *)(hci_data), hci_data_len);
        if(ret != 0) ret = 0xFF;
...
static bool retry_hci_flag = 1;
static uint8_t retry_cnt = 0;
static uint32_t call_hci_cmd_from_uart_handle_last_t_ms = 0;

#define MIN_MS_CALL_HCI_CMD_HANDLE_RETRY     100
#define MAX_RETRY_CALL_HCI_CMD_HANDLE        40
		...
		if (rx_len)
		{
			...
			if(retry_hci_flag == 1){
				if(clock_time_exceed_ms(
						call_hci_cmd_from_uart_handle_last_t_ms, MIN_MS_CALL_HCI_CMD_HANDLE_RETRY)){
					state = app_hci_cmd_from_usb_handle(p->data, rx_len);
					call_hci_cmd_from_uart_handle_last_t_ms = clock_time_ms();
					retry_cnt++;
					DBG_MESH_COMMON_SEND_STR("\n RETRY_CNT: ");
					DBG_MESH_COMMON_SEND_INT(retry_cnt);
				}
				else {
					return -1;
				}
			...
			DBG_MESH_COMMON_SEND_STR("\n state: ");
			DBG_MESH_COMMON_SEND_INT(state);
			...
			if(state != 0xFF)
			{
				...
				DBG_MESH_COMMON_SEND_STR("\n REC: ");
				foreach(i, p->len) {
					DBG_MESH_COMMON_SEND_HEX_ONE_BYTE(p->data[i]);
					DBG_MESH_COMMON_SEND_STR(" ");
				}
				...
				if(clock_time_exceed_ms(tmp_ms, 2000)) {
					cnt = 1;
				}
				else {
					cnt++;
				...
				if(retry_cnt >= MAX_RETRY_CALL_HCI_CMD_HANDLE){
					my_fifo_pop(&hci_rx_fifo);
					retry_hci_flag = 0;
					retry_cnt = 0;
				}
